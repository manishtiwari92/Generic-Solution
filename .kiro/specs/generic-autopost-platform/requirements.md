...
24. WHEN processing in auto mode, THE InvitedClubPlugin SHALL use `configuration.UserId` (the `UserId` field from `InvitedClubConfig`, which maps to `default_user_id` in `generic_job_configuration`) as the `userId` for all `WORKITEM_ROUTE` and `GENERALLOG_INSERT` calls. **Note: The `InvitedClubConfig` model property is named `UserId` (int), NOT `DefaultUserId`. All references in `InvitedClubPlugin` and `InvitedClubPostStrategy` must use `invitedClubConfig.UserId`.** WHEN processing in manual mode, THE InvitedClubPlugin SHALL use the `userId` passed in the manual post request.
...
### Requirement 21: Sevita Plugin — Invoice Posting

**User Story:** As a Sevita operations user, I want the platform to post approved invoices to the Sevita AP system with OAuth2 authentication, line item grouping, and attachment handling, so that vendors can be paid through the Sevita ERP system.

#### Acceptance Criteria

1. THE SevitaPlugin SHALL authenticate to the Sevita AP API using OAuth2 `client_credentials` grant type. The token endpoint URL, `client_id`, and `client_secret` are stored in `client_config_json` under `api_access_token_url`, `client_id`, and `client_secret` fields respectively.
2. THE SevitaPlugin SHALL cache the OAuth2 Bearer token and reuse it until `token_expiration_min` minutes have elapsed, then fetch a new token. The token is fetched by POSTing to `api_access_token_url` with `Content-Type: application/x-www-form-urlencoded` and body `grant_type=client_credentials&client_id={id}&client_secret={secret}`.
3. THE SevitaPlugin SHALL set `SqlHelper.ConnectionString = connectionStr` ONCE at startup (not per-configuration). The per-configuration connection string assignment is NOT used in Sevita — it is commented out in the source. SevitaPlugin MUST NOT reassign `SqlHelper.ConnectionString` per-configuration.
4. WHEN `SevitaPlugin.OnBeforePostAsync` is called, THE SevitaPlugin SHALL load `ValidIds` by executing a multi-statement SQL query using a raw `SqlConnection` + `SqlCommand` directly (NOT via `SqlHelper`): `SELECT Supplier FROM Sevita_Supplier_SiteInformation_Feed; SELECT EmployeeID FROM Sevita_Employee_Feed`. The two result sets SHALL be read using `SqlDataReader.NextResult()` in a single round trip. The resulting `ValidIds` object contains `HashSet<string> VendorIds` and `HashSet<string> EmployeeIds`.
5. THE SevitaPlugin SHALL call `get_sevita_configurations` SP at startup to load configuration. This SP returns, among other fields: `db_error_to_email_address`, `db_error_cc_email_address`, `db_error_email_subject`, and `db_error_email_template`. These four fields SHALL be deserialized into a `DBErrorEmailConfiguration` class within `SevitaConfig`. The `DBErrorEmailConfiguration` class SHALL have properties: `ToEmailAddress` (string), `CcEmailAddress` (string), `EmailSubject` (string), and `EmailTemplate` (string).
6. FOR EACH workitem, THE SevitaPlugin SHALL always attempt to retrieve the invoice image from S3 (Sevita always uses S3, never local file system). WHEN the image is not found (S3 returns null), THE SevitaPlugin SHALL: (a) route the workitem to `FailedPostsQueueId` via `WORKITEM_ROUTE`; (b) write a history record to `sevita_posted_records_history` with `InvoiceRequestJson = ""` and `InvoiceResponseJson = ""`; (c) continue to the next workitem. **This is different from InvitedClub behavior — Sevita DOES write a history record for image-fail workitems.**
7. THE SevitaPlugin SHALL validate each workitem before calling the external API. Validation failures SHALL route the workitem to `FailedPostsQueueId` without calling the external API. Validation rules:
   - Line sum validation: `SUM(lineItem.amount)` must equal `header.InvoiceAmount`.
   - PO record validation (when `is_PO_record = true`): required fields `vendorId`, `invoiceDate`, `invoiceNumber`, `checkMemo`; `vendorId` must be in `ValidIds.VendorIds`; default `checkMemo = "PO#"` if empty.
   - Non-PO record validation (when `is_PO_record = false`): required fields `vendorId`, `employeeId`, `invoiceDate`, `invoiceNumber`, `checkMemo`, `expensePeriod`; both IDs must be in `ValidIds`; `cerfTrackingNumber` required if any line has `naturalAccountNumber = "174098"`.
   - Attachment validation: `fileName`, `fileBase`, `fileUrl`, and `docid` all required on each attachment.
8. THE SevitaPlugin SHALL build line items by grouping detail rows by `alias + naturalAccountNumber`, summing `LineAmount` per group, formatting amount to 2 decimal places, and setting `edenredLineItemId = edenredInvoiceId + "_" + lineItemCount`.
9. THE SevitaPlugin SHALL serialize the `InvoiceRequest` and wrap it in a JSON array: `"[{...}]"`. This wrapped JSON array is the request body sent to the Sevita API.
10. THE SevitaPlugin SHALL POST the invoice payload to the Sevita API using `request.AddParameter("application/json", invoiceRequestJson, ParameterType.RequestBody)` — NOT `AddJsonBody`. This is a Sevita-specific API requirement distinct from InvitedClub's `calculateTax` endpoint which uses `AddJsonBody`.
11. WHEN the invoice POST returns HTTP 201, THE SevitaPlugin SHALL extract `InvoiceId` from the `invoiceIds` property (first property name) of the response JSON and route the workitem to `SuccessQueueId`.
12. WHEN the invoice POST returns HTTP 500, THE SevitaPlugin SHALL use the special error message `"Internal Server error occurred while posting invoice."` and route the workitem to `FailedPostsQueueId`.
13. WHEN the invoice POST returns any other non-201 status, THE SevitaPlugin SHALL extract `recordErrors`, `message`, `invoiceIds`, and `failedRecords` from the response JSON and route the workitem to `FailedPostsQueueId`.
14. THE SevitaPlugin SHALL call `GetAPIResponseTypes` (from `api_response_configuration`) at the start of each `PostData` execution for manual posts to load response type mappings. **Sevita uses `api_response_configuration` only — NOT `sevita_response_configuration`. The `GetSevitaPostResponseTypes` method (which reads `sevita_response_configuration`) is defined in the data layer but is NEVER called in the actual PostData flow. The `postResponseTypes` class member is always empty. The `sevita_response_configuration` table exists in the database but is unused in the current implementation.**
15. THE SevitaPlugin SHALL call `UpdateSevitaHeaderPostFields(@UID)` SP after processing each workitem (both success and failure paths). This SP serves as the `ClearPostInProcessAsync` override for Sevita.
16. THE SevitaPlugin SHALL save history to `sevita_posted_records_history` for every workitem processed (including image-fail workitems — see AC6). WHEN saving history, THE SevitaPlugin SHALL null out the `fileBase` field on all attachments in the request JSON before storing, to prevent large base64 strings in the database. The `fileBase` nulling is performed by parsing `invoiceRequestJson` as a `JArray` (because the payload is wrapped in `[{...}]`), iterating each item's attachments array, and setting `fileBase = null` on each attachment object.
17. WHEN `post_json_path` is configured in `client_config_json`, THE SevitaPlugin SHALL upload the serialized request JSON to S3 at `{post_json_path}/{itemId}_{timestamp}.json` before calling the external API.
18. AFTER processing all workitems in a batch, WHEN any workitems failed, THE SevitaPlugin SHALL send a notification email. The email body is built by generating an HTML table from the failed records list using `GenerateHtmlTable()` (excluding the `IsSendNotification` column), then replacing the `[[AppendTable]]` placeholder in the email template with the generated HTML table. **The placeholder is `[[AppendTable]]` — NOT `#MissingImagesTable#`. This matches the `EmailSender.SendPostNotificationMail` implementation which calls `mailBody.Replace("[[AppendTable]]", tableBody)`.**
19. THE SevitaPlugin data seed migration script SHALL map from `post_to_sevita_configuration` using `configuration_id` as the primary key (NOT `id`). The `UpdateLastPostTime` SP uses `WHERE configuration_id = @configuration_id`. The migration INSERT script for `generic_job_configuration` must correctly reference `configuration_id` from `post_to_sevita_configuration`.
20. THE SevitaPlugin SHALL call `GENERALLOG_INSERT` on both success and failure paths for each workitem.
21. ALL HTTP REST client calls to the Sevita API SHALL use an infinite timeout (`Timeout = -1` in RestSharp).
