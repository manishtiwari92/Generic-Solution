...
- [ ] 10. Implement InvitedClubFeedStrategy
  ...
  - [ ] 10.7 Implement ExportSupplierCsvAsync (call InvitedClub_GetSupplierDataToExport SP, write pipe-delimited CSV to FeedDownloadPath\Supplier\Supplier_{timestamp}.csv)
...
- [ ] 12. Implement InvitedClubPostStrategy
  ...
  - [ ] 12.3 Implement PostInvoiceAsync (POST to PostServiceURL, Content-Type: application/json, Basic Auth, Timeout=-1, expect HTTP 201, extract InvoiceId from response JSON)
  ...
  - [ ] 12.7 Implement ExecuteAsync main loop (for each workitem: set PostInProcess=1, get image, validate RequesterId, build payload, PostInvoice, PostAttachment, PostCalculateTax if UseTax=YES, route to success/fail queue, write history, clear PostInProcess in finally; after loop: send image failure email if any failed)
  - [ ] 12.8 Implement image failure email (to EmailToHelpDesk, use EmailTemplateImageFail, replace #MissingImagesTable# with GenerateHtmlTable(), use EmailSubjectImageFail)
...
- [ ] 16. Implement SevitaPostStrategy
  ...
  - [ ] 16.4 Implement PostInvoiceAsync (POST to InvoicePostURL, Authorization: Bearer {token}, Timeout=-1; HTTP 201: extract InvoiceId from invoiceIds first property name; HTTP 500: special error message; other: extract recordErrors/message/invoiceIds/failedRecords)
