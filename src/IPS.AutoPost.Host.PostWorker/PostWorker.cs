...
namespace IPS.AutoPost.Host.PostWorker;

/// <summary>
/// ECS Fargate background service that continuously polls <c>ips-post-queue</c> for
/// invoice posting jobs and dispatches them via MediatR.
/// </summary>
/// <remarks>
/// <para>
/// Polling behaviour:
...
/// </para>
/// </remarks>
public sealed class PostWorker : BackgroundService
{
    ...
    {
        _logger.LogInformation("PostWorker started. Polling queue: {QueueUrl}", _queueUrl);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogDebug("Received {MessageCount} message(s) from post queue.", messages.Count);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Unexpected error in PostWorker polling loop. Retrying in 5 seconds.");
                await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
            }
        }

        _logger.LogInformation("PostWorker stopped.");
    }
    ...
    private async Task ProcessMessageAsync(Message message, CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "Processing post message. MessageId: {MessageId}, Body: {Body}",
            message.MessageId,
            message.Body);
        ...
        try
        {
            ...
            var command = new ExecutePostCommand
            {
                ...
                ItemIds     = payload.ItemIds ?? string.Empty,
            ...
            _logger.LogInformation(
                "Post command completed. MessageId: {MessageId}, JobId: {JobId}, " +
                "ClientType: {ClientType}, RecordsProcessed: {RecordsProcessed}, " +
                "RecordsSuccess: {RecordsSuccess}, RecordsFailed: {RecordsFailed}",
                message.MessageId,
                payload.JobId,
                payload.ClientType,
        ...
        catch (OperationCanceledException) when (ct.IsCancellationRequested)
        {
            ...
            _logger.LogWarning(
                "Post message processing cancelled during shutdown. MessageId: {MessageId}. " +
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(
                ex,
                "Failed to process post message. MessageId: {MessageId}, JobId: {JobId}. " +
